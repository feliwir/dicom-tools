...
export function DicomJsonValidatorTool() {
  ...
  useEffect(() => {
    ...
    if (!result || result.valid) {
      monacoRef.current.editor.setModelMarkers(model, 'dicom-json-validator', [])
      return
    }
    ...
    monacoRef.current.editor.setModelMarkers(model, 'dicom-json-validator', markers)
  ...
  return (
    <section className="space-y-4 rounded-xl border border-slate-800 bg-slate-900/70 p-4">
      <div className="flex items-center justify-between gap-3">
        ...
        <p className="text-sm text-slate-300">{isValidating ? 'Validating…' : 'Auto validation enabled'}</p>
      </div>
          ...
          {!result.valid && (
            <ul className="mt-3 space-y-2 text-sm text-red-300">
              {result.errors.map((error, index) => (
                <li key={`${error.message}-${index}`} className="rounded bg-red-950/40 p-2">
                  <span className="font-medium">Error {index + 1}:</span> {error.message}
